...
/** Queue for On-Board Green LED1 */
QUEUE_DEFINE(led_1_queue, LED_QUEUE_SIZE);

/** Queue for On-Board Green LED2 */
QUEUE_DEFINE(led_2_queue, LED_QUEUE_SIZE);

/** Queue for On-Board Red LED3 */
QUEUE_DEFINE(led_3_queue, LED_QUEUE_SIZE);
...
void bsp_init(board_t * board) {
  ...
  gpio_ctx_init(&cfg.gpio, GPIO_TYPE_BIND(BTN), GPIO_POL_POSITIVE);
  cfg.press_time = 1000;
  ...
  // Initialize I2C
  i2c_init(&board->i2c_pulse, &(i2c_cfg_t){ .i2c_no = 1 });

  // Initialize I2C
  i2c_init(&board->i2c_accel, &(i2c_cfg_t){ .i2c_no = 2 });
